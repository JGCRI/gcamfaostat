...
# 1.1 Source data options ----
Process_Raw_FAO_Data <- FALSE
## If TRUE, process raw FAO data
## If FALSE, use prebuilt data to load FAO data

## FAO raw data folder directory
DIR_RAW_DATA_FAOSTAT <- "FAOSTAT"
## it also includes mappings or other data assumptions used

# 1.2 Generate output csv options ----
...
# 1.3. Module function inclusion options----
DISABLED_MODULES <- c("aglu_")
## gcamdata specific modules (aglu_) are disabled
## DISABLED_MODULES is an input in the "find_chunks" function
## we can also disable other module functions e.g.,
## "yextension_L100_FoodBalanceSheet", "yfaostat_SUA_FBS_CSVExport"
...
# **************************************----
# ****CONSTANTS LIKELY NEEDED FOR GCAM aglu MODULES ----

if (any("aglu_" %in% DISABLED_MODULES) ) {
  # Default constants in gcamdata (GCAM v7.2) ----
