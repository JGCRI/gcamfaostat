...
module_xfaostat_L200_ExportCSV <- function(command, ...) {
  ...
  if(command == driver.DECLARE_INPUTS) {
    ...
    FAO_an_Stocks %>% distinct(item, item_code, element, unit)


    QCL_AN_PRIMARY_MILK %>% filter(element == "Milk Animals")%>%
      spread(year, value) -> FAO_an_Dairy_Stocks
    FAO_an_Dairy_Stocks %>% distinct(item, item_code, element, unit)

    FAO_an_Stocks %>% bind_rows(
      FAO_an_Dairy_Stocks
    ) -> GCAMDATA_FAOSTAT_AnimalStock_202Regs_22Items_1973to2020

    output_csv_data("GCAMDATA_FAOSTAT_AnimalStock_202Regs_22Items_1973to2020",
                    col_type_nonyear = "icicicc",
                    title = "FAO all meat animal stock and milk animal stock by area_item_year",
                    unit = "Head, 1000 Head or No", code = "QCL",
    ...
    output_csv_data("GCAMDATA_FAOSTAT_ProducerPrice_170Regs_185PrimaryItems_2010to2020",
                    col_type_nonyear = "iciccc",
                    title = "FAO primary Ag and An production quantity and value by area_item_year for prices derivation",
                    unit = "Quantity in tonne; Value in US$", code = "PP",
    ...
    FAO_GDP_Deflators <- PD %>% FAO_AREA_RM_NONEXIST(RM_AREA_CODE = NULL) %>%
      spread(year, value)

    output_csv_data("FAO_GDP_Deflators",
                    col_type_nonyear = "cicici",
                    title = "FAO GDP deflators by country (2015 = 100)",
                    unit = "Unitless", code = "PD",
